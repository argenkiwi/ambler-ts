...
This project has a Zettelkasten — an atomic, explicitly-linked note store — at `.zettelkasten/zettel.db` (SQLite, with full-text search built in and optional semantic embeddings). It exists so design decisions and gotchas compound across sessions instead of being re-discovered every time. Use it via the `zettel-*` deno tasks below; do not read or write `.zettelkasten/zettel.db` directly.
...
```bash
deno task zettel-update <id>   # partial {title?,body?,tags?} via stdin
...
All six tasks print a single JSON object/array to stdout — parse it directly. Search blends keyword (FTS5) and, when a local OpenAI-compatible embeddings host is reachable (default `http://localhost:11434/v1`, model `nomic-embed-text`), semantic similarity — it degrades gracefully to keyword-only if no such host is running.
