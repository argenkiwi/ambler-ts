...
- **Branches to cover**: Every `return new Next(...)` line is one branch; the terminal `return null` is another. List them all before writing any test.
...
- [ ] Every edge path (`return new Next(...)`) has a dedicated test.
