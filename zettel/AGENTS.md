...
This project has a Zettelkasten — an atomic, explicitly-linked note store. Notes are Markdown files with YAML frontmatter under `notes/` (the version-controlled source of truth, editable in Obsidian/HelixNotes or by hand); `.zettelkasten/zettel.db` is a derived, gitignored SQLite index (full-text search, optional semantic embeddings, and the link graph) rebuildable from `notes/` at any time. It exists so design decisions and gotchas compound across sessions instead of being re-discovered every time. Use it via the global `zettel` CLI command below; do not read or write `.zettelkasten/zettel.db` directly. If you ever hand-edit a file under `notes/`, run `zettel reindex` afterward so the index reflects it.

**Before implementing any non-trivial prompt:**

```bash
zettel search "<short summary of the task>"
```
...
```bash
echo '{"title":"<short title>","body":"<what you decided or learned, and why>","tags":["<tag>"],"links":[{"toId":"<id>","relation":"<short phrase>"}]}' | zettel create
```
...
```bash
echo '{"title":"..."}' | zettel update <id>   # partial {title?,body?,tags?} via stdin
zettel delete <id>
```
...
```bash
zettel link <fromId> <toId> "<relation phrase>"
```
