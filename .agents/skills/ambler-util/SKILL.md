---
name: ambler-util
description: Creates or extracts utility modules into the utils/ directory of an Ambler project. Use this whenever a core's defaultUtils needs external npm/jsr dependencies, contains logic reusable across multiple cores, or has implementations too complex to inline — even if the user just says "add a util", "extract this helper", "share this function", "create a utility", or "this code is getting too long for the core". When a user creates a core that imports an npm package or has complex async logic, proactively suggest using this skill.
metadata:
  author: leandro
  version: "1.2"
---
...
Create a `utils/<name>.ts` file when a core's `defaultUtils` contains:

1. **External dependencies** — any `npm:` or `jsr:` import (e.g., `npm:ollama`, `npm:marked`)
2. **Reusable logic** — code that is or could be used by more than one core
3. **Complex implementations** — functions with significant logic (error handling, retries, connection caching, multi-step operations) that clutter the core file
...
- **Cores that will use it**: Which core(s) will import this utility?
...
- **Match the core's `Utils` type** — exported function signatures must match exactly what the consuming core's `Utils` type declares, so they can be passed in directly.
...
Every exported function gets a TSDoc comment. This helps both humans reading the code and agents using the utility in future cores understand what it does without having to trace the implementation.
...
Import the utility in the core file and use it in `defaultUtils`. The `Utils` type stays unchanged — only the implementation in `defaultUtils` changes.
...
- [ ] Exported function signatures match the core's `Utils` type exactly.
...
- [ ] The core's `defaultUtils` delegates to the utility function.
- [ ] No `ambler.ts` imports appear in the utility file.
- [ ] Core tests still use mocked utils — they need no changes.
