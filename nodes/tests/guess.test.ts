...
Deno.test("guess should accept a valid new letter", async () => {
  ...
  const result = await factory({ onValid: "check", onInvalid: "guess" }, utils)(
    initialState,
  );
...
Deno.test("guess should reject an empty input", async () => {
  ...
  assert(
    warning.includes("single letter"),
    "Should warn about single letter requirement",
  );
});
...
Deno.test("guess should reject a non-letter input", async () => {
  ...
  assertEquals(result[1].guessedLetters, []);
  assert(
    warning.includes("alphabetic"),
    "Should warn about alphabetic requirement",
  );
});
...
Deno.test("guess should reject an already-guessed letter", async () => {
  ...
  assert(
    warning.includes("already guessed"),
    "Should warn about already guessed letter",
  );
});
