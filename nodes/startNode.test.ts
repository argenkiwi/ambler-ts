...
Deno.test("startNode should transition to onSuccess with 0 if input is empty", async () => {
  ...
  const captureNext: Nextable<StartNode.State> = async (s) => { capturedState = s; return null; };
...
Deno.test("startNode should transition to onSuccess with input number", async () => {
  const initialState: StartNode.State = { count: 0 };
  let capturedState: StartNode.State | undefined;
  const captureNext: Nextable<StartNode.State> = async (s) => { capturedState = s; return null; };
...
Deno.test("startNode should transition to onError if input is invalid", async () => {
  const initialState: StartNode.State = { count: 123 };
  let capturedState: StartNode.State | undefined;
  const captureSuccess: Nextable<StartNode.State> = async (_s) => { return null; };
  const captureError: Nextable<StartNode.State> = async (s) => { capturedState = s; return null; };
