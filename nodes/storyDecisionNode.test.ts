...
Deno.test("storyDecisionNode should return null when storyPages is empty", async () => {
...
Deno.test("storyDecisionNode should transition with unchanged state when no checkboxes found", async () => {
  ...
  const result = await StoryDecisionNode.create({ onDecisionMade: captureNext }, utils)(state);
...
Deno.test("storyDecisionNode should return null when readLine returns null", async () => {
  const state = {
    ...baseState,
    storyPages: ["You stand at a crossroads.\n1. [ ] Go left\n2. [ ] Go right"],
  };
...
Deno.test("storyDecisionNode should mark selected checkbox and transition onDecisionMade", async () => {
  let capturedState: StoryDecisionNode.State | undefined;
  const captureNext: Nextable<StoryDecisionNode.State> = async (s) => {
    capturedState = s;
    return null;
...
Deno.test("storyDecisionNode should retry on invalid input then accept valid input", async () => {
  let capturedState: StoryDecisionNode.State | undefined;
  const captureNext: Nextable<StoryDecisionNode.State> = async (s) => {
    capturedState = s;
    return null;
