...
Deno.test("storyIntroNode should return null when identity readLine returns null", async () => {
...
Deno.test("storyIntroNode should return null when placement readLine returns null", async () => {
  let call = 0;
  const utils: StoryIntroNode.Utils = {
...
Deno.test("storyIntroNode should return null when circumstances readLine returns null", async () => {
  let call = 0;
  const utils: StoryIntroNode.Utils = {
    readLine: async (_msg) => {
...
Deno.test("storyIntroNode should set identity, placement, and circumstances trimmed", async () => {
  ...
  let call = 0;
  const utils: StoryIntroNode.Utils = {
    readLine: async (_msg) => ["  Ada  ", "  London, 1842  ", "  Inventing the engine  "][call++],
    print: () => {},
  };

  const result = await StoryIntroNode.create({ onIntroComplete: captureNext }, utils)(baseState);
