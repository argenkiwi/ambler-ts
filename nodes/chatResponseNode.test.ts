...
Deno.test("chatResponseNode should send messages to chat, print reply, and append to history", async () => {
  ...
  const next = await ChatResponseNode.create({ onPrompt: capture }, utils)(initialState);
...
Deno.test("chatResponseNode should pass the full message history to the chat util", async () => {
  const initialState: ChatResponseNode.State = {
    messages: [
      { role: "user", content: "Hello" },
