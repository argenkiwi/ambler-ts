...
Deno.test("modelSelectNode should return null when readLine returns null", async () => {
...
Deno.test("modelSelectNode should transition with unchanged state when input is NaN", async () => {
  ...
  const result = await ModelSelectNode.create({ onSelect: captureNext }, utils)(baseState);
...
Deno.test("modelSelectNode should transition with unchanged state when index is out of range", async () => {
  let capturedState: ModelSelectNode.State | undefined;
  const captureNext: Nextable<ModelSelectNode.State> = async (s) => {
    capturedState = s;
    return null;
...
Deno.test("modelSelectNode should set selectedModel when valid index is given", async () => {
  let capturedState: ModelSelectNode.State | undefined;
  const captureNext: Nextable<ModelSelectNode.State> = async (s) => {
    capturedState = s;
    return null;
